...
if (!isset ($_SESSION['Employee_name'])) {
    ...
    {
        $sql = "SELECT * FROM customer WHERE user_phone LIKE '%$searchTerm%'";
        $result = mysqli_query($conn, $sql);
        return $result;
    }
            ...
            <tbody>
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td>" . $row['branch_chosen'] . "</td>";
                        echo "<td>" . $row['user_birthday'] . "</td>";
                        echo "<td>" . $row['user_phone'] . "</td>";
                        echo "<td>" . $row['prefered_contact_method'] . "</td>";
                        // echo "<td><a href='historyVisit.php?id=" . $row['UserID'] . "'>View History</a></td>";
                        echo "<td><a href='historyVisit.php?id=" . $row['UserID'] . "&name=" . urlencode($row['user_name']) . "'>View History</a></td>";
